...
watchWindows(function(window) {
  ...
  function willHandle(search) {
    ...
    // Check if there's a keyworded bookmark for the first keyword
    if (PlacesUtils.getURLAndPostDataForKeyword(keywords[0])[0] != null) {
      // Only let the location bar handle single-term keywords to allow searches
      // to be selected from the autocomplete results
