...
watchWindows(function(window) {
  ...
  // The pref doesn't exist since Firefox 49, so default to unified behavior
  let targetIndex = 1;
  try {
    // We want index 0 for non-unified and index 1 for unified (default)
    targetIndex = ~~Services.prefs.getBoolPref("browser.urlbar.unifiedcomplete");
  }
  catch(ex) {}
  ...
  change(popup, "_appendCurrentResult", function(orig) {
    return function() {
      ...
      if (selectNext) {
        selectNext = false;
        async(_ => gURLBar.controller.handleEnter(true));
      }
    };
  });
